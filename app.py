...
# ========== DATABASE PATH - USE /tmp FOR RENDER ==========
# Render allows writing to /tmp directory
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
DB_NAME = os.path.join(BASE_DIR, 'ecoshop.db')

# For Render, also try /tmp
RENDER_DB_PATH = '/tmp/ecoshop.db'
...
def get_db():
    """Get database connection - ensures database exists"""
    db_path = get_db_path()
...
if not os.path.exists(db_path):
    ...
    try:
        ...
        if count == 0:
            ...
            print(f"✅ Database ready! {count} products found at {db_path}")
